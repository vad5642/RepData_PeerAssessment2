...
Since event type format is not specified it is necessary to do some preprocessing to merge the same event records. For example, there are events with types Frost/Freeze, FROST/FREEZE and FROST\\FREEZE. It is obviosly that these are the same type of event. So it is necessary to convert the case and substitute punctuation character with spaces. Of cause this simple preprocessing does not guarantied that all duplications are merged but results get better.
...
```
This figure shows that flash floods and thunderstorm winds cause the largest property damages. It should be noted that since the available data are not tidy, types "flood", "flash flood", etc are separate observation. They should be merged for more accurate data-driven conclusions.

The most severe weather event in terms of crop damage is the drought. In the last half century, it has caused more than 10 billion dollars damage. Other severe crop-damage-causing events are floods and hails.
